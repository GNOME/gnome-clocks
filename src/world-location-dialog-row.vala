...
private class AddClockRow : Gtk.ListBoxRow {
    ...
    public AddClockRow (GWeather.Location location, Face world) {
        ...
        if (weather_time_zone != null) {
            var time_zone = new GLib.TimeZone (((GWeather.Timezone)weather_time_zone).get_tzid ());
            var date_time = local_time.to_timezone (time_zone);
            var local_offset = local_time.get_utc_offset () - date_time.get_utc_offset ();
            string time_diff_message = Row.get_time_difference_message (local_offset);
            string time_zone_name = ((GWeather.Timezone)weather_time_zone).get_name ();
